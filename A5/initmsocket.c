    ...
    while (1) {
        for(int i=0; i<MAXSOCKETS; i++) {
            ...
            if (SM[i].send_time[SM[i].swnd_markers[0]].tv_usec != 0 && curr_time.tv_usec - SM[i].send_time[SM[i].swnd_markers[0]].tv_usec > (((int)T) * 1000000)) {
                ...
                int iter = SM[i].swnd_markers[0];
                while (iter != (SM[i].swnd_markers[1]+1)%SWND) {
                    if (SM[i].send_status[iter] == 1 && SM[i].send_time[iter].tv_usec == 0) {
                        struct sockaddr_in addr;
                        addr.sin_family = AF_INET;
                        addr.sin_port = htons(SM[i].dest_port);
                        addr.sin_addr.s_addr = inet_addr(SM[i].dest_ip_addr);
                        ...
                        printf("the sequence number getting sent : %d \n", SM[i].swnd.seq_no[iter]);
                        printf("got sent %s \n", temp_buf);
                        gettimeofday(&curr_time, NULL);
...
int main() {
    ...
    for(int i=0; i<MAXSOCKETS; i++){
        ...
        SM[i].send_seq_no = 1;
        SM[i].recv_seq_no = 1;
        SM[i].swnd_markers[0] = 0; SM[i].swnd_markers[1] = (MAXWNDW-1) < MAXSEQNO/2 ? (MAXWNDW-1) : MAXSEQNO/2;
        SM[i].rwnd_markers[0] = 0; SM[i].rwnd_markers[1] = (MAXWNDW-1) < MAXSEQNO/2 ? (MAXWNDW-1) : MAXSEQNO/2;
